            ...
            (function () {
                ...
                for (var j = 0; j < algorithms.length; j++) {
                    ...
                    if ($('[name="' + current.name + '-switch"]').prop("checked")) {
                        ...
                        var algoInst = {
                            name: current.name,
                            instance: current.type.create(cusrrent.param)
                        };
                ...
                $("#list").append('<div id="' + uid + '" class="progress-bar">' +
                    '<b>' + escapeHtml(f.name) + ' <span class="progress-bar"></span></b>' +
                ...
                progressiveRead(f,
                    function (data, pos, file) {
                        ...
                        if (progress > lastprogress) {
                            ...
                            if (took > 0.1) // Only show progressbar after 100ms so it won't show for very small files
                                $("#" + uid + " .progress-bar").progressbar({
                                    value: progress
                                });

                            $("#" + uid + " .progress-bar").html('(' +
                    ...
                    function (file) {
                        ...
                        var results = '<div class="card"><table>';
                        ...
                        results += '<span class="card-text">Time taken: ' + digits(took, 2) + 's @ ' + bytes2si(file.size / took, 2) + '/s</span><br />';

                        $("#" + uid).append(results);

                        $("#" + uid + " .progress-bar-stipped")
                            .hide('slow');
